...
// 环境光 - 基础照明，避免完全黑暗的区域
const ambientLight = new THREE.AmbientLight(0xffffff, 1.5); // 降低强度，避免过曝
...
loader.load(
    ...
    (gltf) => {
        ...
        guitarModel.rotation.z = (Math.PI / 4)*-1;// X轴旋转45°
...
function addGuitarControls() {
    ...
    // 添加自动旋转速度控制
    guitarFolder.add({ autoRotateSpeed }, 'autoRotateSpeed', 0, 0.02, 0.001)
        .name('自动旋转速度')
        .onChange(value => {
...
motionFolder.add(curveIntensity, 'value', 0.1, 2, 0.1).name('曲线强度')
    .onChange(value => {
        curveIntensity = value;
    });
motionFolder.add({ historyLimit }, 'historyLimit', 3, 30, 1).name('平滑度')
    .onChange(value => {
        historyLimit = value;
    });
motionFolder.add(inertia, 'value', 0.85, 0.98, 0.01).name('惯性大小')
    .onChange(value => {
        inertia = value;
    });
